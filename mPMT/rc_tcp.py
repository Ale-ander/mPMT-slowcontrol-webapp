...
class RunControl:
    ...
    def cmd_read_sensors(self, args):
        sensors_measurses = {}
        tlaData = self.tla.readAll()
        V1 = (tlaData[0] * 3 / 1000)  # +5V
        sensors_measurses.update({'V_5V': round(V1, 3)})
        V2 = (tlaData[2] * 2 / 1000)  # +3.3V
        sensors_measurses.update({'V_3V3': round(V2, 3)})
        I1 = tlaData[1] / 1000  # Imon1
        sensors_measurses.update({'I_poeA': round(I1, 3)})
        I2 = tlaData[3] / 1000  # Imon2
        sensors_measurses.update({'I_poeB': round(I2, 3)})
        P1 = V1 * I1
        P2 = V1 * I2
        sensors_measurses.update({'P_poeA': round(P1, 3)})
        sensors_measurses.update({'P_poeB': round(P2, 3)})

        bmeData = self.bme.readAll()
        sensors_measurses.update({'T_C': round(bmeData[0], 2)})
        ...
        if self.bmeExt is not None:
            ...
            sensors_measurses.update({'Pext': round(bmeData[1], 2)})
            if self.bmeExt.readId() == 0x96:  # BME280
